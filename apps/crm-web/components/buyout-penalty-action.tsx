...
export function BuyoutPenaltyAction(props: {
  buyoutId: string;
  overdueDays: number;
  ...
  const canManualPenalty = useHasPermission("buyouts.manual_penalty");
  const canPayPenalty = useHasPermission("buyouts.pay_penalty");
  const [isPending, startTransition] = useTransition();
  const [manualAmountRubles, setManualAmountRubles] = useState("0");
  const [manualReason, setManualReason] = useState("Просрочка выкупа");
  ...
  function submitManualPenalty() {
    ...
    startTransition(async () => {
      try {
        const response = await fetch(`${getApiBase()}/buyouts/${props.buyoutId}/penalties/manual`, {
          method: "POST",
          credentials: "include",
          headers: {
        ...
        setStatus("Ручной штраф по выкупу начислен. Обновляю карточку...");
        router.refresh();
      } catch (requestError) {
        setError(requestError instanceof Error ? requestError.message : "Не удалось начислить ручной штраф по выкупу.");
      }
    });
  }

  function submitPenaltyPayment() {
    // Buyout penalties follow the same split as rental penalties: accrual and money collection are different operations.
    ...
    startTransition(async () => {
      try {
        const response = await fetch(`${getApiBase()}/buyouts/${props.buyoutId}/penalties/${selectedPenalty.id}/pay`, {
          method: "POST",
          credentials: "include",
          headers: {
            "Content-Type": "application/json"
        ...
        setPaymentError(requestError instanceof Error ? requestError.message : "Не удалось провести оплату штрафа по выкупу.");
      }
    });
  }
    ...
    <article className="surface-card">
      ...
      <div className="penalty-payment-block">
        <div className="penalty-payment-head">
          ...
          <span>Денежный факт фиксируется отдельно от начисления штрафа.</span>
        </div>
        ...
        {activePenalties.length > 0 ? (
          <>
            <div className="penalty-payment-grid">
              ...
              <label className="action-field penalty-payment-full">
                ...
                <textarea
                  ...
                  placeholder="Например: клиент закрыл штраф отдельным переводом"
                  disabled={isPending}
                />
              </label>
            </div>
            ...
            {!canPayPenalty ? <p className="route-card-note">Недостаточно прав для фиксации оплаты штрафа выкупа.</p> : null}
      ...
      <label className="action-field">
        ...
        <textarea
          className="action-input action-textarea"
          placeholder="Например: менеджер подтвердил штраф по просрочке платежа"
          rows={3}
          value={manualComment}
      ...
      {!canManualPenalty ? <p className="route-card-note">Недостаточно прав для ручного штрафа по выкупу.</p> : null}
    </article>
  );
}
